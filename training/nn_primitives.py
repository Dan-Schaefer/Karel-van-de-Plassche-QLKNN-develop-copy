...
def model_to_json(name, trainable, feature_names, target_names,
    ...
    placeholder = lambda x,y,z: x
    trainable['prescale_factor'] = scale_factor.astype('float64').to_dict()
    trainable['prescale_bias'] = scale_bias.astype('float64').to_dict()
    trainable['feature_min'] = dict(placeholder(train_set._features.min(), scale_factor, scale_bias).astype('float64'))
    trainable['feature_max'] = dict(placeholder(train_set._features.max(), scale_factor, scale_bias).astype('float64'))
    trainable['feature_names'] = feature_names
    trainable['target_names'] = target_names
    trainable['target_min'] = dict(placeholder(train_set._target.min(), scale_factor, scale_bias).astype('float64'))
    trainable['target_max'] = dict(placeholder(train_set._target.max(), scale_factor, scale_bias).astype('float64'))
